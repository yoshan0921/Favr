 ...
 * Get all the documents of a given collection
 * @param {string} collectionPath
 * @param {Array} filterConditions - an array of objects that contain the conditions to filter the documents
 * @returns An array containing objects that represent each document
 */
async function getAllWithFilter(collectionPath, filterConditions) {
  try {
    let reference = collection(firestore, collectionPath);

    // Create query using condition object (key, operator, value)
    // Example: { key: 'age', operator: '>', value: 18 }
    if (filterConditions) {
      for (let condition of filterConditions) {
        reference = query(reference, where(condition.key, condition.operator, condition.value));
      }
    }
